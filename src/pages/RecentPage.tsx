...
export function RecentPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const data = await api.getProducts({ trending: false, limit: 20 });
      setProducts(data);
    } catch (err) {
    ...
    <div className="w-full max-w-none min-w-0 px-4 md:px-0">
      <div className="mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-zinc-900 dark:text-white flex items-center gap-3">
          <Clock className="w-7 h-7 sm:w-8 sm:h-8 text-orange-500" />
          Recently Launched
        </h1>
        <p className="text-zinc-600 dark:text-zinc-400 mt-2">
          Fresh products from the community
        </p>
      </div>
      ...
      {loading ? (
        ...
        /* Recent Launches Onboarding Empty State - NO fake data */
        <div className="text-center py-16 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6">
          ...
          <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-2">No startups launched yet</h3>
          <p className="text-zinc-500 dark:text-zinc-400 text-sm max-w-sm mx-auto">
            Be the pioneer builder! Tell the world what you launched today.
          </p>
        </div>
      )}
    </div>
